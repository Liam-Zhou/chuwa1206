...
24. what is the @configuration and @bean?
    @Configuration: This annotation is used in Spring to denote a class as a source of bean definitions. Classes marked with @Configuration indicate that they can contain methods annotated with @Bean and can be processed by the Spring container to generate bean definitions and service requests.

@Bean: The @Bean annotation is used at the method level and tells Spring that a method produces a bean to be managed by the Spring container. It is typically used within classes annotated with @Configuration. The method's return value becomes a bean and is registered within the application context, making it available for dependency injection into other components, services, or controllers within the Spring application.
25.  How do you handle the exception in Spring?
    Spring provides a comprehensive approach to handle exceptions through the @ControllerAdvice and @ExceptionHandler annotations. @ControllerAdvice allows you to handle exceptions across the whole application, not just to an individual controller. You can define a class annotated with @ControllerAdvice and use @ExceptionHandler methods within it to handle exceptions globally. This approach helps in centralizing exception handling logic and keeping controllers clean.
26.  How do you do the validations in Spring? And list some validation annotaitons
    you know.
27. You can use annotations from the javax.validation.constraints package in your model classes to enforce rules. Spring then automatically applies these validations when processing HTTP requests. Some common validation annotations include:

@NotNull: The annotated element must not be null.
@NotEmpty: The annotated element must not be null or empty (applied to strings, collections, maps, arrays).
@NotBlank: The annotated element must not be null and must contain at least one non-whitespace character (applied to strings).
@Size(min=, max=): The annotated element size must be within the specified boundaries.
@Min(value): The annotated element must be a number whose value must be equal or greater than the specified minimum.
@Max(value): The annotated element must be a number whose value must be equal or less than the specified maximum.
@Email: The annotated element must be a valid email address.
28.  What is the actuator, list some endpoints it provides?
     Spring Boot Actuator provides production-ready features to help you monitor and manage your application. It exposes various endpoints allowing you to check the health, metrics, configurations, and more.