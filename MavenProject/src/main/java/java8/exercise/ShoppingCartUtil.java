...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        Optional<String> items = Optional.ofNullable(customer)
    ...
    public static double getTotalPrice(Optional<Customer> customer) {
        double total =  customer.map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> !items.isEmpty())
                .map(items -> items.stream().mapToDouble(Item::getPrice).sum())
                .orElse(0.0);
        double total1 =  customer.map(c -> c.getShoppingCart())
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        String firstItemName= Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(cart -> cart.getItems())
                .filter(items -> !items.isEmpty())
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        String firstName = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems)
                .filter(items -> items.isEmpty())
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseThrow(() -> new EmptyCartException("Shopping cart cannot be empty"));
        return firstName;
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        // write you code
        Optional<List<Item>> itemsOption = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
        // 因为可能有对象，但是实际可能仅仅是一个空list,里面并没有实际的值
        if (itemsOption.isPresent() && !itemsOption.isEmpty()) {
