...
public class ShoppingCartUtil {
    //https://github.com/B1gO/chuwa-eij-tutorial/tree/main/02-java-core/src/main/java/com/chuwa/tutorial/t06_java8/exercise
    // 定义一个备选商品列表
     ...
     * The method returns an Optional<String>, which will be empty if any of the intermediate steps result in a null value or if the shopping cart is empty.
     */
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(item -> item.getName())
                .orElse(ALTERNATIVE_ITEMS.stream().findAny().get());
    }
    ...
    public static void printItemsInCart(Customer customer) {
        Optional<List<Item>> itemsOptional = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
        itemsOptional.ifPresent(items -> items.stream().map(Item::getName).forEach(System.out::println));
    }
