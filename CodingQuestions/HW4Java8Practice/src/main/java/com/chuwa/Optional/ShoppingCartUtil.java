...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        /*
        // not correct! customer or shopping cart may be null!
...
//        Optional<Item> item = Optional.ofNullable(customer
//                .getShoppingCart()
//                .getItems()
//                .get(0));
//
//        return item.map(Item::getName)
//                .orElseGet(ShoppingCartUtil::getRandomAlternativeItem);
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseThrow(() -> new EmptyCartException("Empty cart or non existing cart"));
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        ...
        if(items.isEmpty() || items.get().isEmpty()){ //java 11 can use optional.isEmpty()
