...
public class ShoppingCartUtil {
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return Optional.ofNullable(customer)
                ...
                .map(items -> items.get(0))
                .map(Item::getName)
                .orElseThrow(()->new EmptyCartException("shopping cart is null or empty"));
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        // shoppingCartOpt is Optional.List<Item> or Optional.empty
        Optional<List<Item>> shoppingCartOpt = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);
        if(shoppingCartOpt.isPresent() && !shoppingCartOpt.get().isEmpty()){
            System.out.println("shopping cart exists and has item(s)");
        }
        else{
            System.out.println("shopping cart is null or empty\"");
        }
    }
...
//        if(shoppingCartOpt.isPresent() && !shoppingCartOpt.get().isEmpty()){
//            System.out.println(shoppingCartOpt.get());
//        }
//        else{
//            System.out.println("shopping cart is null or empty\"");
//        }
        shoppingCartOpt.ifPresent(items -> {
            if(!items.isEmpty()){
    ...
    public static void main(String[] args) {
        ...
        shoppingcart.setItems(List.of(new Item("a", 0),
                new Item("a", 1),new Item("a", 2),new Item("a", 3)));
        Customer customer = new Customer();
        customer.setShoppingCart(shoppingcart);
        Optional<Customer> optional1 = Optional.of(customer);
        ShoppingCartUtil.printItemsInCart(customer);
//        Optional<List<Item>> optional2 = optional1.map(Customer::getShoppingCart).map(ShoppingCart::getItems);
//        System.out.println(ShoppingCartUtil.getFirstItemName(null));
//        System.out.println(ShoppingCartUtil.getTotalPrice(optional1));
