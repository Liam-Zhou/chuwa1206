...
public class ShoppingCartUtil {
     ...
     * 如果Optional对象非空且包含的客户有购物车，计算购物车中所有商品的总价，并返回该值。如果购物车为空或不存在，返回默认值0.0。
     * @param customer
     * @return
     */
    ...
    public static void printItemsInCart(Customer customer) {
        // write your code
        Optional<List<Item>> itemsOptinal = Optional.ofNullable(customer)
                .map(Customer::getShoppingCart)
                .map(ShoppingCart::getItems);

        itemsOptinal.ifPresent(items -> {
            if (!items.isEmpty()) {
                System.out.println("购物车有商品");
