...
public class ShoppingCartUtil {
    ...
    public static Optional<String> getFirstItemName(Customer customer) {
        if (customer != null && customer.getShoppingCart() != null && !customer.getShoppingCart().getItems().isEmpty()) {
            return Optional.of(customer.getShoppingCart().getItems().get(0).getName());
        } else {
    ...
    public static String getFirstItemNameWithAlternative(Customer customer) {
        return getFirstItemName(customer).orElse(getRandomAlternativeItem());
    }
    ...
    public static String getFirstItemNameOrThrowException(Customer customer) throws EmptyCartException {
        return getFirstItemName(customer).orElseThrow(EmptyCartException::new);
    }
    ...
    public static void checkItemsInCart(Customer customer) {
        Optional<String> firstItemName = getFirstItemName(customer);
        if (firstItemName.isPresent()) {
    ...
    public static void printItemsInCart(Customer customer) {
        Optional<ShoppingCart> cart = Optional.ofNullable(customer).map(Customer::getShoppingCart);
        cart.ifPresent(c -> c.getItems().forEach(item -> System.out.println(item.getName())));
    }
